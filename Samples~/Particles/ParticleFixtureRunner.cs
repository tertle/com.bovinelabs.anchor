namespace BovineLabs.Anchor.Particles.Sample
{
    ...
    public sealed class ParticleFixtureRunner : MonoBehaviour
    {
        ...
        [SerializeField] private VisualTreeAsset runtimeFixture;
        private ParticleSamplePresenter runtimePresenter;
        ...
        private void Awake()
        {
            ...
            if (Array.IndexOf(Environment.GetCommandLineArgs(), "--particle-sample") >= 0)
            {
                this.ShowRuntimeSample();
            }
            else
            {
                this.BindFixture();
        ...
        private void ShowRuntimeSample()
        {
            this.document.rootVisualElement.Clear();
            this.runtimeFixture.CloneTree(this.document.rootVisualElement);
            this.runtimePresenter = new ParticleSamplePresenter(this.document.rootVisualElement);
        }
        ...
        private IEnumerator Start()
        {
            ...
            var runtimeArgument = Array.IndexOf(args, "--particle-runtime-results");
            if (runtimeArgument >= 0)
            {
                ...
                var output = Path.GetFullPath(args[runtimeArgument + 1]);
                Directory.CreateDirectory(output);
                ScreenCapture.CaptureScreenshot(Path.Combine(output, "runtime-samples.png"));
                yield return null;
                yield return null;
                this.runtimePresenter.Dispose();
                this.runtimePresenter = null;
                yield return ParticleRuntimeMeasurements.Run(this.document.rootVisualElement, Path.GetFullPath(args[runtimeArgument + 1]));
                Application.Quit();
                yield break;
            }
